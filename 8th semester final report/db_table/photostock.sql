-- rename database
--CREATE DATABASE photostock2 / DROP DATABASE photostock3

create database if not exists photostock;
use photostock;
...
alter table product add constraint fk_desginer_product foreign key (desginer_id) references desginer(desginer_id);
alter table product add constraint fk_category_product foreign key (category_id) references category(category_id);
alter table product auto_increment=1200;
desc product;
-- alter table product  drop mediumImage, drop smallImage, drop footer;
-- alter table product  drop longDescription, drop shortDescription;

-- alter table product change total_stock_item total_stock int(10);
-- alter table product change current_stock_item current_stock int(10);

-- alter table product add longDescription text after keyword;
-- alter table product  modify uploadDate varchar(255);
-- alter table product  modify keyword varchar(5000);

create table if not exists product_rating_review(
  ...
  constraint pk_product_rating_review primary key (rating_review_id)
);
alter table product_rating_review add constraint fk_product_product_rating_review foreign key (product_id) references product(product_id);
alter table product_rating_review auto_increment=1300;
...
create table if not exists product_comment(
  ...
  product_id int(11) not null,
  user_id int(11) not null,
  constraint pk_product_comment primary key (comment_id)
);
alter table product_comment add constraint fk_product_product_comment foreign key (product_id) references product(product_id);
alter table product_comment add constraint fk_user_product_comment foreign key (user_id) references user(user_id);
alter table product_comment auto_increment=1400;
...
alter table comment_like add constraint fk_product_comment_like foreign key (comment_id) references product_comment(comment_id);
...
alter table comment_dislike add constraint fk_product_comment_dislike foreign key (comment_id) references product_comment(comment_id);
...
alter table shopping_cart add constraint fk_product_shopping_cart foreign key (product_id) references product(product_id);
...
-- for billing, credit card, order and invoice product
...
create table if not exists invoice_product(
  invoice_product_id int(11) auto_increment,
  item_count int(5) default 0,
  totalPrice decimal(10, 2) default 0,
  ...
  credit_card_id int(11) not null,
  user_id int(11) not null,
  constraint pk_invoice_product primary key (invoice_product_id) 
);
alter table invoice_product add constraint fk_credit_card_invoice_product foreign key (credit_card_id) references credit_card(credit_card_id);
alter table invoice_product add constraint fk_user_invoice_product foreign key (user_id) references user(user_id);
alter table invoice_product auto_increment=3300;
desc invoice_product;


create table if not exists invoice_product_history(
  invoice_product_history_id int(11) auto_increment,
  quantity int(5) default 1,

  product_id int(11) not null,
  invoice_product_id int(11) not null,
  constraint pk_invoice_product_history primary key (invoice_product_history_id) 
);
alter table invoice_product_history add constraint fk_product_history foreign key (invoice_product_id) references invoice_product(invoice_product_id);
alter table invoice_product_history add constraint fk_product_invoice_product_history foreign key (product_id) references product(product_id);
alter table invoice_product_history auto_increment=3400;
...
alter table order_item add constraint fk_invoice_product_order foreign key (invoice_product_id) references invoice_product(invoice_product_id);
...
alter table purchase_item add constraint fk_product_purchase_item foreign key (product_id) references product(product_id);
